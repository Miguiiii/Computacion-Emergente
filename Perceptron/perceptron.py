...
class Adeline:
    ...
    def train(self):
        self._ask_training_data()
        for whatever in range(self.epoch):
            self.pred = []
            for i in len(self.model[0]): #realiza las predicciones con los pesos y el bias actuales
                yt=sum([self.model[j][i]*self.pesos[j] for j in range(self.nw)])+self.bias
                yt=self.activate(yt)
                self.pred.append(yt)
            self.err = [self.model[-1][i]-self.pred[i] for i in range(len(self.pred))]
